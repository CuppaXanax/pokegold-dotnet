    ...
    let stepWith (walkable: int -> int -> bool) (collId: int -> int -> byte) (buttons: Buttons) (p: PlayerState) : PlayerState =
        let next =
            ...
            | motion ->
                ...
                // or hop translates the player; a turn or bump holds position.
        ...
        // OBJECT_STEP_FRAME ticks once per frame an object performs a step/bump
        // action, and InitStep never resets it — so the leg counter free-runs across
        // tiles (feet alternate smoothly) and a held bump keeps marching. Standing
        // and Turning aren't stepping actions, so it holds.
        match next.Motion with
        | Walking
        | Hopping
