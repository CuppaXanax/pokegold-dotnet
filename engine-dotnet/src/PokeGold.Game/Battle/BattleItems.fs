...
module BattleItems =
    ...
    let private statusCures : Map<string, StatusCondition -> bool> =
        Map.ofList
            [ "ANTIDOTE", (function Poison | BadPoison _ -> true | _ -> false)
    ...
    let private selectedMovePpItems = Set.ofList [ "ETHER"; "MAX_ETHER" ]
    let private allMovePpItems = Set.ofList [ "ELIXER"; "MAX_ELIXER" ]
    let private directItems = Set.ofList [ "X_ATTACK"; "X_DEFEND"; "X_SPEED"; "X_SPECIAL"; "X_ACCURACY"; "GUARD_SPEC"; "DIRE_HIT"; "POKE_DOLL" ]
    ...
    let private restoreHp item (mon: BattleMon) : BattleMon option =
        if mon.Hp <= 0 || mon.Hp >= mon.MaxHp then
            ...
            |> Map.tryFind item
            |> Option.map (fun data ->
                let hp = if data.Param < 0 then mon.MaxHp else min mon.MaxHp (mon.Hp + data.Param)
                { mon with Hp = hp })

    let private cureStatus item (mon: BattleMon) : BattleMon option =
        if mon.Hp <= 0 then
            None
        elif item = "FULL_HEAL" || item = "HEAL_POWDER" then
    ...
    let tryUse item target state =
        ...
        | item, PartyTarget partyIndex when Set.contains item reviveItems -> updatePartyMon partyIndex (revive item) state
        | item, PartyTarget partyIndex when Set.contains item hpRestoreItems -> updatePartyMon partyIndex (restoreHp item) state
        | item, PartyTarget partyIndex when Map.containsKey item statusCures -> updatePartyMon partyIndex (cureStatus item) state
