...
/// Parses the GSC audio script (`audio/music/*.asm`, `audio/sfx.asm`) into `Song`s.
/// Notes are written as pitch symbols (`C_`..`B_`) and control flow as labels; both
/// are resolved here so the synth sees only numbers and command indices.
module SongParser =
    ...
    /// Load a single-song music file (e.g. "audio/music/azaleatown.asm").
    let loadMusicFile (relativePath: string) : Song =
        let pf = parseFile (Assets.readText relativePath)
        match pf.Headers with
        | h :: _ -> toSong pf h
        | [] -> failwithf "No song header found in %s" relativePath

    // audio/sfx.asm holds every SFX; parse it once and slice songs out by name.
    let private sfxFile = lazy (parseFile (Assets.readText "audio/sfx.asm"))

    /// Load a named SFX (e.g. "Sfx_Menu") from audio/sfx.asm.
    let loadSfx (name: string) : Song =
        let pf = sfxFile.Value
        match pf.Headers |> List.tryFind (fun h -> h.Name = name) with
        | Some h -> toSong pf h
        | None -> failwithf "SFX '%s' not found in audio/sfx.asm" name
