...
module ConformanceLedger =
    ...
    let all: LedgerEntry list =
        [
          ...
          yield! many ItemFieldMenu ImplementedApproximate [ CriticalPathJohto; RequiredFor100Percent ] "PackScene builds action menus from item field-menu metadata: UI-001 runtime tests cover OLD_ROD through Pack, source water/surf guards, generated fish-group selection, byte bite/slot thresholds, and no-bite text; UI-002 PackUseGiveTests cover source status cures and Full Restore through the Party picker. PackTests cover ITEMMENU_CLOSE cant-toss key items and PackUseGiveTests cover use/give plus ITEMMENU_NOUSE deferred-use gating. Fishing animation, phone swarms, revives, vitamins, PP items, and other key-item effects remain future work."
