...
module EmitMaps =
    ...
    let private flyPoint (point: FlyPoint) : string =
        sprintf
            "{ Landmark = %s; Spawn = %s; Flag = %s; MapId = %s; X = %d; Y = %d }"
            (str point.Landmark) (str point.Spawn) (str point.Flag) (str point.MapId) point.X point.Y

    /// The full generated `Maps.Generated.fs` source.
    let render (maps: GeneratedMap list) (spawnPoints: (string * string * int * int) list) (flyPoints: FlyPoint list) : string =
        ...
        sb.AppendLine("        |]") |> ignore
        sb.AppendLine() |> ignore
        sb.AppendLine("    /// Source-defined Fly destinations in flypoints.asm order.") |> ignore
        sb.AppendLine("    let flyPoints : FlyPoint[] =") |> ignore
        sb.AppendLine("        [|") |> ignore
        for point in flyPoints do
            sb.AppendLine(sprintf "            %s" (flyPoint point)) |> ignore
        sb.AppendLine("        |]") |> ignore
