...
- Current validation: `dotnet build .\src\PokeGold.Game` and `dotnet test
  .\tests\PokeGold.Tests` (1311/1311) are green from `engine-dotnet`.
...
- Production battle staging rejects missing/invalid combatants; stable identities
  ...
  persist exact EXP/stat EXP with source modifier rounding; crossed-level
  processing and blackout remain critical-path work.
...
1. Complete BAT-010 through BAT-019: finish crossed-level processing,
   evolution, rewards, and blackout.
