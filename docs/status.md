...
- Current validation: `dotnet build .\src\PokeGold.Game` and `dotnet test
  .\tests\PokeGold.Tests` (1312/1312) are green from `engine-dotnet`.
...
- Production battle staging rejects missing/invalid combatants; stable identities
  ...
  and level evolution is deferred to victorious cleanup; blackout remains
  critical-path work.
...
1. Complete BAT-011 through BAT-019: finish rewards, move decisions, evolution
   methods, and blackout.
