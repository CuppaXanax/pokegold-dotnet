...
- ✅ **OVR-008 — Complete Waterfall.**
  - Source `CheckMapCanWaterfall` activation requires facing up at a waterfall/current tile; ascent repeats its forced upward movement until the first non-waterfall cell, and waterfall flow forces descent.
  - Tohjo Falls runtime coverage proves facing rejection, animated ascent and descent, stable water landings, and retained surfing state.
...
- ⬜ **SCR-001 — Fix battle-script control flow.**
  ...
  - ✅ `checkjustbattled` now returns the transient trainer-script state that `endifjustbattled` consumes, with both branch values and one-shot consumption covered by `ScriptTests`.
  - ✅ `startbattle` now resumes capture as source `WIN = 0` and wild RUN as `DRAW = 2`; loss remains an explicit blackout/abort path. `reloadmapafterbattle` Mom/Bill dispatch remains open.
...
- ⬜ **SCR-003 — Implement required side-system command stubs.**
  ...
  - ✅ `Writeobjectxy` exists only in the native `SeenByTrainerScript` wrapper, which the port neither generates nor dispatches; generated map scripts contain no use. The unreachable typed/parser/VM cases were removed so a future live use fails generation instead of silently no-oping.
